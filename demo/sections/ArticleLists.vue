...
const LISTS: ListDefinition[] = [
  {
    ...
    description: 'サムネイル・見出し・抜粋・メタ情報を揃えた標準レイアウト。',
  },
  {
    key: 'rounded',
    name: 'RoundedList',
    component: RoundedList,
    description: '角丸のカードで柔らかい印象に。ブログや読み物向け。',
  },
  {
    key: 'artistic',
    name: 'ArtisticList',
    component: ArtisticList,
    description: '画像を大きく見せる構成。ビジュアル主体のメディア向け。',
  },
  {
    key: 'tile',
    name: 'TileList',
    component: TileList,
    description: '画像の上にテキストを重ねるタイル型。',
  },
  {
    key: 'simple',
    name: 'SimpleList',
    component: SimpleList,
    description: '装飾を抑えた最小構成。サイドバーや関連記事に。',
  },
  {
    key: 'row',
    name: 'RowList',
    component: RowList,
    description: '横並びの 1 行レイアウト。検索結果や一覧ページ向け。',
  },
  {
    key: 'news',
    name: 'NewsList',
    component: NewsList,
    description: '日付とカテゴリを前面に出したニュース向け。',
  },
  {
    key: 'entertainment',
    name: 'EntertainmentList',
    component: EntertainmentList,
    description: '賑やかな見せ方。エンタメ・キャンペーン系に。',
  },
  {
    key: 'gallery',
    name: 'GalleryList',
    component: GalleryList,
    description: '画像を敷き詰めるギャラリー表示。',
  },
  {
    key: 'grid',
    name: 'GridList',
    component: GridList,
    description:
      'カードごとに大きさと向きが変わるグリッド。columnNumber が必須。',
    needsColumnNumber: true,
  },
]
...
const THEME_OPTIONS = LISTS.map((list) => ({
  label: list.name,
  value: list.key,
}))
...
const currentList = computed(
  () => LISTS.find((list) => list.key === theme.value) ?? LISTS[0]
)
const articles = computed(() =>
  hasImage.value ? ARTICLES : ARTICLES_WITHOUT_IMAGE
)
...
const resetColors = () =>
  Object.assign(colors, prefersDark() ? DARK_COLORS : LIGHT_COLORS)

/** サイドバーのレイアウト名リンク（#/article-list#gallery）でテーマを切り替える */
const applyHash = () => {
  const anchor = window.location.hash.split('#')[2] ?? ''
  if (LISTS.some((list) => list.key === anchor)) {
    theme.value = anchor
  }
}
...
onMounted(() => {
  ...
  window
    ...
    .addEventListener('change', resetColors)
...
onUnmounted(() => {
  window
    .matchMedia('(prefers-color-scheme: dark)')
    .removeEventListener('change', resetColors)
  window.removeEventListener('hashchange', applyHash)
})

const SETUP_CODE = computed(
  () => `<script setup lang="ts">
...
<template>
  <div :class="$style.page">
    <section :class="$style.intro">
      <h3 :class="$style.introTitle">記事一覧プレイグラウンド</h3>
      <p :class="$style.introText">
        WordPress REST API（<code>?_embed</code>
        付き）のレスポンス配列をそのまま
        <code>articles</code> に渡すだけで一覧が組み上がります。
    ...
    <section :class="['demo-preview', $style.controls]">
      ...
      >
        <span>表示オプション</span>
        <span :class="[$style.toggleArrow, { [$style.opened]: isControlsOpen }]"
          >▾</span
        >
      </button>
        ...
        <div :class="$style.controlsInner">
          ...
          <div :class="$style.controlRow">
            <LabeledCheckbox
              ...
              label="ピックアップ表示"
            />
            <LabeledCheckbox v-model="useAuthor" name="author" label="著者" />
            <LabeledCheckbox
              v-model="useCategory"
              name="category"
              label="カテゴリ"
            />
            <LabeledCheckbox v-model="useTag" name="tag" label="タグ" />
            <LabeledCheckbox
              v-model="hasImage"
              name="image"
              label="アイキャッチあり"
          ...
          <div :class="$style.controlRow">
            ...
            <button type="button" :class="$style.reset" @click="resetColors">
              配色をリセット
            </button>
          </div>
        </div>
    ...
    <section :class="$style.previewBlock">
      ...
      <div :class="$style.preview" :style="colors">
        <component
    ...
    <section :class="$style.codeBlock">
      <h3 :class="$style.previewTitle">この設定のコード</h3>
